...
def sr_get_fbank_features(all_df, num_samples):
    sigs_all = []
    y_word_id = []
    y_word = []
    y_actual_word = []
    for row in tqdm(all_df.values):
        ...
        if len(sig) > num_samples:
            ...
            y_word.append(word)
            y_actual_word.append(actual_word)
    x = np.zeros((len(y_word_id), 99, 26), dtype=np.float32)
    idx = 0
    for s in tqdm(sigs_all):
        fbank_feat = logfbank(s)
        # if fbank_feat.shape != (99, 26):
        #     print(fbank_feat.shape, s.shape)
        x[idx, :, :] = StandardScaler().fit_transform(fbank_feat)
        idx += 1
    lb = LabelBinarizer()
